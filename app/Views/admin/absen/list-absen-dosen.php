<div id="dataMahasiswa" class="card-body table-responsive pb-5">
   <?php if (!empty($data)) : ?>
         ...
         <tbody>
            ...
            <?php foreach ($data as $value) : ?>
               ...
               <tr>
                  ...
                  <td>
                     <?php if (!$lewat && (user()->toArray()['is_superadmin'] == 1 || user()->toArray()['is_superadmin'] == 3) && (!(str_contains(strtolower($value['nama_dosen']), "yuyun") || str_contains(strtolower($value['nama_dosen']), "ahmad") || str_contains(strtolower($value['nama_dosen']), "faisal") || str_contains(strtolower($value['nama_dosen']), "sarmin") || str_contains(strtolower($value['nama_dosen']), "ichsan")))) : ?>
                        <button data-toggle="modal" data-target="#ubahModal" onclick="getDataKehadiran(<?= $value['id_presensi'] ?? '-1'; ?>, <?= $value['id_dosen']; ?>)" class="btn btn-info p-2" id="<?= $value['id_dosen']; ?>">
